"""
Phase 8 tests: Master Reference integrity, Phase 0-7 regression protection,
repository cleanliness for the Phase 7-8 boundary, phase-boundary audit (no
Phase 9+ implementation leaked in), dependency discipline, and
docs/PHASE_08_EVIDENCE_OBJECT_AND_CITATION_ARCHITECTURE.md completeness.
"""
...
EXPECTED_CONFIG_FILES_THROUGH_PHASE_7 = {
...
# Phases beyond the one now in progress (Phase 8). "ingest", "chunker",
...
# ---------------------------------------------------------------------------
# Phase 0-7 artifacts remain intact
# ---------------------------------------------------------------------------


def test_phase_00_through_07_docs_still_present_and_intact():
    docs = [
...
def test_phase_0_through_7_yaml_contracts_still_valid_and_unchanged_in_shape():
    taxonomy = yaml.safe_load((CONFIG_DIR / "domain_taxonomy.yaml").read_text(encoding="utf-8"))
    assert taxonomy["taxonomy_version"] == "1.0.0"

    hybrid_contract = yaml.safe_load((CONFIG_DIR / "hybrid_retrieval_contract.yaml").read_text(encoding="utf-8"))
    assert hybrid_contract["contract_version"] == "1.0.0"

    hybrid_schema = yaml.safe_load((CONFIG_DIR / "hybrid_result_schema.yaml").read_text(encoding="utf-8"))
    assert hybrid_schema["schema_version"] == "1.0.0"


def test_phase_tracker_phases_0_through_7_remain_done_and_all_phases_present():
...
def test_retrieval_source_files_are_untouched_by_phase_8():
...
def test_full_pipeline_through_evidence_is_importable_and_functional():
    import sys
...
# ---------------------------------------------------------------------------
# Repository cleanliness for the Phase-7-through-Phase-8 boundary
# ---------------------------------------------------------------------------


def test_config_directory_still_contains_the_phase_0_through_7_files():
    actual = {p.name for p in CONFIG_DIR.glob("*.yaml")}
    assert EXPECTED_CONFIG_FILES_THROUGH_PHASE_7.issubset(actual), (
        f"config/ is missing Phase 0-7 files: {EXPECTED_CONFIG_FILES_THROUGH_PHASE_7 - actual}"
    )


def test_evidence_config_files_exist():
    assert (CONFIG_DIR / "evidence_contract.yaml").is_file()
    assert (CONFIG_DIR / "evidence_schema.yaml").is_file()


def test_ingestion_chunking_and_retrieval_source_directories_untouched_by_phase_8():
...
def test_evidence_source_directory_contains_expected_files():
...
def test_no_phase_9_or_10_directories_exist_yet():
    ...
    for forbidden in ("citations", "generation", "answers", "indexes", "index"):
        assert not (REPO_ROOT / forbidden).exists(), (
            f"'{forbidden}/' would imply Phase 9+ functionality, which Phase 8 must not create"
        )
...
def test_no_llm_agent_or_web_framework_imports_anywhere_in_evidence_source():
    # Checks actual import statements only - not comments/docstrings (Phase
    # 8's own source and docs extensively, and permissibly, discuss what it
    # does NOT do).
    ...
    for py_file in (SRC_DIR / "evidence").glob("*.py"):
...
def test_no_new_dependency_declared_beyond_phase_6_baseline():
    ...
    # Phase 8 adds ZERO new dependencies - identical to Phase 6/7's set.
    ...
    assert expected.issubset(declared_packages), (
        ...
        f"Phase 8 must add zero new dependencies beyond Phase 6's baseline, got: {declared_packages}"
    )
...
def test_evidence_fixtures_reuse_existing_synthetic_builders_not_new_ones():
    text = (REPO_ROOT / "tests" / "_evidence_fixtures.py").read_text(encoding="utf-8")
    assert "from _bm25_fixtures import" in text
    assert "from _dense_fixtures import" in text
    assert "from _hybrid_fixtures import" in text


def test_no_generated_answer_field_exists_anywhere_in_evidence_models():
    text = (SRC_DIR / "evidence" / "models.py").read_text(encoding="utf-8")
...
def phase_08_doc_text() -> str:
    path = DOCS_DIR / "PHASE_08_EVIDENCE_OBJECT_AND_CITATION_ARCHITECTURE.md"
    assert path.is_file()
    return path.read_text(encoding="utf-8")


def test_phase_08_doc_exists_and_nonempty(phase_08_doc_text: str):
    assert len(phase_08_doc_text) > 0


@pytest.mark.parametrize("section_heading", REQUIRED_DOC_SECTIONS)
def test_phase_08_doc_has_required_section(phase_08_doc_text: str, section_heading: str):
    assert section_heading in phase_08_doc_text


def test_phase_08_doc_distinguishes_evidence_id_from_chunk_id(phase_08_doc_text: str):
    assert "chunk_id remains the underlying" in phase_08_doc_text or "distinct from" in phase_08_doc_text


def test_phase_08_doc_discloses_version_and_url_gaps(phase_08_doc_text: str):
    assert "[DEFERRED]" in phase_08_doc_text
    assert "not currently carried forward" in phase_08_doc_text or "not propagated" in phase_08_doc_text


def test_phase_08_doc_states_phase_9_and_10_boundaries(phase_08_doc_text: str):
    assert "Phase 9" in phase_08_doc_text
    assert "Phase 10" in phase_08_doc_text
    assert "citation validation" in phase_08_doc_text.lower()
